...
<body>
  ...
  <script>
    // ---------- CONFIG ----------
    // Put your WebSocket server URL here (ws:// or wss://) to enable realtime sync across browsers.
    // If left blank, app will work locally using localStorage only.
    const WS_URL = '';
    ...
    // more preset teams for first-time users
    const initialTeams = [
    ...
    function handleRemoteMessage(msg){
      if(msg.type === 'sync' && msg.state){
        // naive merge: if remote has teams/history, adopt them (simple approach). You can customize merging logic.
    ...
    function renderTeams(){
      ...
      Array.from(document.querySelectorAll('.use')).forEach(btn => btn.addEventListener('click', e=>{
        ...
        // quick fill: if home empty set home, else set away
    ...
    function goal(side, src = 'manual'){
      ...
      // broadcast remote event if connected
    ...
    // keyboard shortcut: G to score random
